...
def corner(xs, weights=None, labels=None, show_titles=False, title_fmt=".2f",
    ...
    trdim = 0.22 * factor   # size of top/right margin
    ...
    if fig is None:
        ...
        fig, axes = pl.subplots(K, K, figsize=(7.2,7.2))
    else:
        try:
    ...
    for i, x in enumerate(xs):
        ...
        n, b, p = ax.hist(x, weights=weights, bins=kwargs.get("bins", 20),
...
def hist2d(x, y, *args, **kwargs):
    ...
    #X = np.linspace(extent[0][0]-0.1*extent[0][0], extent[0][1]+0.1*extent[0][1], bins + 1)
    #Y = np.linspace(extent[1][0]-0.1*extent[1][0], extent[1][1]+0.1*extent[1][1], bins + 1)
    try:
    ...
    V = 1.0 - np.exp(-0.5 * np.arange(1.0, 3.1, 1.0) ** 2)
    #V = 1.0-np.array([0.68,0.99,0.997])
    Hflat = H.flatten()
    ...
    if plot_datapoints:
		...
		ax.plot(x, y, "o", color=color, ms=0.8, zorder=-1, alpha=0.1,
			rasterized=True)
		#if plot_contours:
    ...
    #extent[0][0] -= 0.1*extent[0][0]
    #extent[1][1] += 0.1*extent[0][0]
    #extent[0][1] += 0.1*extent[0][0]
    #extent[1][0] -= 0.1*extent[0][0]
