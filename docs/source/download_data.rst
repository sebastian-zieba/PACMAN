...
.. note::
    ...
    The recommended way to download HST data for PACMAN is using the ``astroquery`` workflow described in :ref:`astroquery_visits`.
    This page describes alternative manual download methods.
...
Go to the `HST MAST search tool <https://mast.stsci.edu/search/ui/#/hst>`_.

.. image:: media/download/mast1.png

Enter 13021 for Proposal ID and click on search.
All files associated with the program will then be shown in the following table.
...
You can find more information on how to download the files with ftp here:
...
You can access the data using the ``ftplib`` module in python. Here's an example script which you can also find on GitHub. It will download the data into the current directory.
...
Using the MAST search tool
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

An alternative is to download it directly after searching for the program files with the `HST MAST search tool <https://mast.stsci.edu/search/ui/#/hst>`_.
...
Go to the bottom right and select Rows per Page = 100, then select the 100 files and click on DOWNLOAD DATA (100 DATASETS).
A small window opens on the website. Go to the EXTENSIONS drop down list and only select ima. Click on Start DOWNLOAD
