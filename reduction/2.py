...
def plot_trace(d, ancil):
    ...
    if ancil.trace_image_output:
        if not os.path.isdir('config/trace/'):
            os.makedirs('config/trace/')
        plt.savefig('config/trace/{0}.png'.format(i))
...
for i, f in enumerate(ancil.files[0:105]):                         #ancil.files only contains the spectra not the di
    ...
    if ancil.all_image_output:
        if not os.path.isdir('config/pics/'):
            os.makedirs('config/pics/')
        plt.savefig('config/pics/background{0}.png'.format(i))
    ...
    if ancil.all_image_output:
        if not os.path.isdir('config/pics/'):
            os.makedirs('config/pics/')
        plt.savefig('config/pics/boxes{0}.png'.format(i))
    if ancil.all_image_show:
        plt.show()
    plt.close()
    ...
    #for ii in range(1):
        ...
        if ancil.all_image_output:
            if not os.path.isdir('config/pics/'):
                os.makedirs('config/pics/')
            plt.savefig('config/pics/uptheramp{0}-{1}.png'.format(i,ii))
        if ancil.all_image_show:
            plt.show()
        plt.close()
