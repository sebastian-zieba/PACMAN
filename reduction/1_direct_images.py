...
for i, file in enumerate(files_di):
	...
	if (ima[0].header['filter'] == ancil.filter):
		...
		if ancil.diagnostics==True:
			...
			plt.colorbar()
			if ancil.direct_image_output == True:
				if not os.path.isdir('diagnostic_plots/images/'):
					os.makedirs('diagnostic_plots/images/')
				plt.savefig('diagnostic_plots/images/{0}.png'.format(i))
			#plt.show()
			plt.close()
