...
def run10(eventlabel, workdir: Path, meta=None):
    ...
    for i, file in enumerate(tqdm(meta.files_di, desc='Determining Source Positions for Direct Images', ascii=True)):
        ...
        LTV1 = ima[1].header['LTV1'] + int(abs(ima[1].header['CRPIX1'] - sp[1].header[
            'CRPIX1']))  # X offset to get into physical pixels + difference in CRPIX1 between direct and spectral image
        LTV2 = ima[1].header['LTV2'] + int(abs(ima[1].header['CRPIX1'] - sp[1].header[
            'CRPIX1']))  # Y offset to get to physical pixels + difference in CRPIX1 between direct and spectral image
